...
$(document).ready(function() {
	...
	var options = {
		...
		'titlePosition' : 'out',
		'colors' : ['#A52A2A', '#A52A2A'],
		'vAxis' : { 'textPosition' : 'out', 'gridlines' : { 'count' : 6 }, "viewWindow" : { "min": 0 } },
		'hAxis' : { 'slantedTextAngle' : 45, 'maxAlternation' : 1, "viewWindow" : { "min": 0 } },
		'width': 400,
		'height': 170,
		'legend' : { 'position' : 'none'} ,
		'backgroundColor' : 'transparent',
...
<?	
	...
	private function getMycoImage() {
		...
		//$soeg->SetDBData('Svampefund.fp7','MycoKeyImageTable', '1');
	...
	private function getTaxonInfo() {
		global $serverIP, $webCompanionPort;
		$soeg=new FX($serverIP,$webCompanionPort);  
		//$soeg->SetDBData('Svampefund.fp7','MycoTaxon', '1');
	...
	private function getHostInfo() {
		global $serverIP, $webCompanionPort;
		$soeg=new FX($serverIP,$webCompanionPort);  
		//$soeg->SetDBData('Svampefund.fp7','Host', 'all');
	...
	private function getFundCount() {
		$SQL='select count(*) from atlasFund where _DKIndex='.$this->DKIndex.' and AtlasForumVali="Godkendt"';;
		$row=$this->getRow($SQL);
		return $row['count(*)'];
	}
	...
	public function draw() {
		...
		echo '<div style="background-color:#EBE8DF;margin-top:10px;">';
